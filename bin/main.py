...
def phone_alive(User):
    ...
    超时3次则认为机器无法连接，短信通知
    """
    log_factory.DEBUGGER.debug(f"进入phone_alive函数")
    ...
    if config[User]['Connect_Type'] == 'Wifi':
        ...
        if client.remote_connect(ip, port):
            log_factory.DEBUGGER.info(f"{name}设备连接成功！，ip地址{ip}，端口{port}")
        else:
            log_factory.DEBUGGER.warn(f"{name}设备连接失败！稍后5秒重试！")
            time.sleep(5)
            if not client.remote_connect(ip, port):
                log_factory.DEBUGGER.error(f"{name}设备重试连接失败!！")
                # send_sms()
        ...
        if client.device(Series_Strings) != None:
            log_factory.DEBUGGER.info(f"{name}设备连接成功！，序列号{Series_Strings}")
        else:
            log_factory.DEBUGGER.warn(f"{name}设备连接失败！稍后5秒重试！")
            time.sleep(5)
            if client.device(Series_Strings) == None:
...
def phone_admin(User):
    log_factory.DEBUGGER.debug(f"进入phone_admin函数")
    config = configparser.ConfigParser()
    config.read(BASE_DIR + '/conf/config.ini')
...
def main():
    "主函数入口"
    log_factory.DEBUGGER.debug(f"开始判断今天是否需要打卡。。。")
    if is_workday(datetime.today()):
        log_factory.DEBUGGER.debug(f"重启adb服务器中...")
        os.system('adb kill-server')
        time.sleep(5)
        os.system('adb start-server')
        log_factory.DEBUGGER.debug(f"读取配置中。。。")
        config = configparser.ConfigParser()
        config.read(BASE_DIR + '/conf/config.ini')
        User_Num = config.getint('Users', 'User_Num')
        # 进行随机等待
        sleep_time = random.randint(0,config.getint('Config', 'sleep_time')*60)
        log_factory.DEBUGGER.info(f"等待时间{sleep_time}")
        time.sleep(sleep_time)
        start = time.time()
        log_factory.DEBUGGER.debug(f"开始多进程,当前时间为 {start}")
        p = Pool(int(config['Config']['pool']))
        # 添加线程
        for i in range(1, User_Num+1):
            p.apply_async(phone_admin, args=('User'+str(i),))  # 非常重要！！！需要在参数后面添加逗号，参数需要以元组的形式传递，并在最后一个参数后面加上 ,号
        log_factory.DEBUGGER.debug('等待所有子进程完成。')
        # close 和 join 是確保主程序結束後，子程序仍然繼續進行
        p.close()
        p.join()        # 调用join之前，先调用close函数，否则会出错。执行完close后不会有新的进程加入到pool,join函数等待所有子进程结束
        end = time.time()
        log_factory.DEBUGGER.info("结束时间为{},总共用时{}秒".format(end, (end - start)))
    else:
        log_factory.DEBUGGER.info("今天不打卡！")

if __name__ == '__main__':
    log_factory.DEBUGGER.debug(f"函数开始,当前母进程: {os.getpid()}")
